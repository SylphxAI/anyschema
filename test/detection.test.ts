...
function createMockValibotSchema() {
	return {
		...
		async: false,
		expects: 'string',
		_run: (dataset: unknown, _config: unknown) => ({ typed: true, value: dataset }),
	}
}
...
function createMockYupSchema() {
	return {
		...
		isValid: async (_data: unknown) => true,
		isValidSync: (_data: unknown) => true,
	}
}
...
function createMockIoTsSchema() {
	return {
		...
		encode: (a: unknown) => a,
		validate: (u: unknown, _c: unknown) => ({ _tag: 'Right', right: u }),
	}
}
...
function createMockEffectSchema() {
	return {
		...
		annotations: {},
		pipe: (..._args: unknown[]) => createMockEffectSchema(),
	}
}
...
describe('Cross-Detection (No False Positives)', () => {
	...
	for (const { name: targetName, create, check: targetCheck } of schemas) {
		it(`${targetName} should only be detected by its own check`, () => {
			const schema = create()
			// Verify target's own check works
			expect(targetCheck(schema)).toBe(true)
