...
describe('AnySchema Protocol v1', () => {
		...
		it('should take priority over Standard Schema', () => {
			...
			const dualProtocolSchema = {
				...
				'~validate': (data: unknown): ValidationResult<string> =>
					typeof data === 'string' ? { success: true, data } : { success: false, issues: [{ message: '' }] },
				'~standard': {
					version: 1,
	...
	describe('JSON Schema Support', () => {
		...
		it('should handle complex JSON Schema', async () => {
			...
			const addressSchema = createSchema<Address>({
				...
				validate: (data) => {
					const d = data as any
					if (typeof d?.street === 'string' && typeof d?.city === 'string' && typeof d?.zipCode === 'string') {
	...
	describe('Async Validation', () => {
		it('should use async validation when available', async () => {
			...
			const schema = createSchema<string>({
				vendor: 'test-lib',
				validate: (data) =>
					typeof data === 'string' ? { success: true, data } : { success: false, issues: [{ message: 'sync' }] },
				validateAsync: async (data) => {
					asyncCalled = true
					await new Promise((r) => setTimeout(r, 5))
					return typeof data === 'string' ? { success: true, data } : { success: false, issues: [{ message: 'async' }] }
				},
			})
	...
	describe('Type Inference', () => {
		it('should correctly infer output type', () => {
			...
			// Type test: InferOutput<typeof schema> should be { id: number; name: string }
	...
	it('AnySchema Protocol should work alongside Standard Schema', () => {
		...
		const standardSchema = {
			'~standard': {
				version: 1 as const,
				vendor: 'standard-lib',
				validate: (d: unknown) => (typeof d === 'string' ? { value: d } : { issues: [{ message: '' }] }),
			},
		}
